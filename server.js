...
const server = http.createServer((request, response) => {
	if (request.method === "GET" && request.url === "/api/health") {
		sendJson(response, 200, { ok: true, service: "web-stack-lab", events: events.length });
		return;
	}
	if (request.method === "POST" && request.url === "/api/events") {
		...
		request.on("end", () => {
			try {
				...
				events.push({ type: event.type.slice(0, 40), message: event.message.slice(0, 200), occurredAt: event.occurredAt || new Date().toISOString() });
				sendJson(response, 201, { ok: true, totalEvents: events.length });
			} catch (error) {
